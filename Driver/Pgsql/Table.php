...
class Table extends AbstractPDO
{
    ...
    public function join($target, $join, $req = array(), $type = 'INNER')
    {
        if ($target && $join) {
            ...
            if (isset($join[0]) && $join[0]) {
                $join = $this->parseSchemas($this->getSchemas()) . '.' . $this->parseKey($join[0]);
            }
     ...
     * @return self
     */
    public function isContainsBy($field, $value)
    {
        return $this->whereOperat(self::isContainsBy, $field, $value);
    }
    ...
    public function insert($data)
    {
        ...
        foreach ($data as $key => $val) {
            if (!empty($ft[$table . '_' . $key])) { // 根据表字段过滤无效key
                if (is_array($val) && isset($val[0]) && 'exp' == $val[0]) {
                    ...
                    // 跟据表字段处理数据
                    if (strpos($ft[$table . '_' . $key], '[]') !== false) {
                        $val = $this->toPGArray($val, str_replace('[]', '', $ft[$table . '_' . $key]));
                    } else {
        ...
        // 兼容数字传入方式
        $sql = 'INSERT INTO ' . $this->parseSchemas($this->getSchemas()) . '.' . $this->parseTable($table) . ' (' . implode(',', $fields) . ') VALUES (' . implode(',', $values) . ')';
    ...
    {
        $table = $this->getTable();
        $sql = 'UPDATE ' . $this->parseSchemas($this->getSchemas()) . '.' . $this->parseTable($table);
        $ft = $this->getFieldType($table);
        $set = array();
        foreach ($data as $key => $val) {
            if (!empty($ft[$table . '_' . $key])) { // 根据表字段过滤无效key
    ...
    public function truncate($sure = false)
    {
        if ($this->getTable() && $sure === true) {
            $sqlStr = "TRUNCATE TABLE " . $this->parseSchemas($this->getSchemas()) . '.' . $this->parseTable($this->getTable());
